<x-admin-layout>
    ...
    <form method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data"
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $book->title) }}" class="w-full border-gray-300 rounded-lg">
            @error('title') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">ISBN</label>
            <input type="text" name="isbn" value="{{ old('isbn', $book->isbn) }}" class="w-full border-gray-300 rounded-lg">
        </div>
    ...
    @error('category_id') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
        ...
        <div>
            ...
            <input type="number" name="stock" value="{{ old('stock', $book->stock) }}" min="0" class="w-full border-gray-300 rounded-lg">
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">Sinopsis</label>
            <textarea name="synopsis" rows="4" class="w-full border-gray-300 rounded-lg">{{ old('synopsis', $book->synopsis) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">Cover Buku</label>
            @if ($book->cover_image_path)
                <img src="{{ Storage::url($book->cover_image_path) }}" class="w-24 h-32 object-cover rounded mb-2">
            @endif
        ...
        <button type="submit" class="px-4 py-2 bg-gray-800 text-white rounded-lg">Simpan Perubahan</button>
    </form>
</x-admin-layout>