...
if ($request->author_id === 'new' && $request->filled('new_author_name')) {
    $author = \App\Models\Author::create(['name' => $request->new_author_name]);
...
if ($request->category_id === 'new' && $request->filled('new_category_name')) {
    $category = \App\Models\Category::create([
        ...
        'slug' => \Illuminate\Support\Str::slug($request->new_category_name),
    ]);
    ...
    public function update(Request $request, Book $book)
{
    ...
    // WAJIB taruh di sini, SEBELUM $book->update($data)
    if ($request->author_id === 'new' && $request->filled('new_author_name')) {
        $author = \App\Models\Author::create(['name' => $request->new_author_name]);
        $data['author_id'] = $author->id;
    }
    ...
    $book->update($data); // ini HARUS paling bawah
